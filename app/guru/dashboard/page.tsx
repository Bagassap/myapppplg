...
function SiswaRow({ item, index }: { item: any; index: number }) {
  ...
  const barColor = p >= 80 ? "#ACEC00" : p >= 60 ? "#013FF6" : "#f43f5e";
  const pBadge =
    p >= 80
  ...
  const avatarBg = index % 2 === 0 ? "bg-[#00182E] text-[#ACEC00]" : "bg-[#013FF6]/10 text-[#013FF6]";
  ...
  return (
    <div className="flex items-center gap-3.5 p-3.5 rounded-2xl bg-slate-50 hover:bg-white hover:shadow-md border border-transparent hover:border-slate-200 transition-all duration-200 cursor-default">
      <div className={`w-10 h-10 rounded-full ${avatarBg} flex items-center justify-center text-sm font-bold shrink-0`}>
        {initial}
      </div>
      ...
      <div className="hidden sm:flex gap-1">
        {[
          { v: hadir, l: "hadir", cls: "bg-[#ACEC00]/10 text-[#00182E]" },
          { v: izinS, l: "izin", cls: "bg-[#013FF6]/8 text-[#013FF6]" },
          { v: alfaS, l: "alfa", cls: "bg-rose-50 text-rose-700" },
        ].map((c) => (
          <span key={c.l} className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-lg ${c.cls}`}>
            {c.v} {c.l}
          </span>
        ))}
      </div>
      <span className={`text-xs font-black px-2.5 py-1 rounded-full shrink-0 ${pBadge}`}>
        {p}%
      </span>
    </div>
  );
                ...
                {!loading && (
                  <span
                    className={`text-xs font-bold px-3 py-1 rounded-full ${
                      pct >= 80 ? "bg-[#ACEC00]/15 text-[#00182E]" : "bg-rose-100 text-rose-700"
                    }`}
                  >
                  ...
                  {bestSiswa && (
                    <div className="mt-4 flex flex-col gap-2 border-t border-slate-100 pt-4">
                      <div className="flex items-center gap-2">
                        <div className="w-9 h-9 rounded-2xl bg-[#013FF6]/10 flex items-center justify-center shrink-0">
                          <Award size={16} className="text-[#013FF6]" />
                        </div>
                        <div>
                          <p className="text-xs text-slate-400">Rata-rata kehadiran</p>
                          <p className="text-xl font-black text-slate-900 leading-none">{pct}%</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 bg-[#ACEC00]/10 rounded-xl px-3 py-2">
                        ...
                        <span className="text-xs text-[#00182E] truncate font-medium">
                          Terbaik: <strong>{bestSiswa.siswa || bestSiswa.tempatPKL}</strong>
                        </span>
                      </div>
                      {worstSiswa && worstSiswa !== bestSiswa && (
                        <div className="flex items-center gap-2 bg-rose-50 rounded-xl px-3 py-2">
                          ...
                          <span className="text-xs text-rose-700 truncate">
                            Perhatian: <strong>{worstSiswa.siswa || worstSiswa.tempatPKL}</strong>
                          </span>
                        </div>
                      )}
                    </div>
              ...
              <div className="flex items-center justify-between mb-5">
                ...
                {!loading && (
                  <div className="flex gap-1.5">
                    {[
                      { v: stats.totalSiswaPKL, l: "Total", cls: "bg-[#013FF6]/10 text-[#013FF6]" },
              ...
              {loading ? (
                ...
                <>
                  ...
                  <div className="grid grid-cols-4 gap-2 bg-slate-50 rounded-2xl p-3">
                    {[
                      { v: stats.totalSiswaPKL, l: "Total", cls: "bg-[#013FF6]/10 text-[#013FF6]", icon: <Users size={13} /> },
