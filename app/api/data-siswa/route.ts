...
export async function GET(req: NextRequest) {
    ...
    const user = session.user as { id?: string; role?: string; name?: string; username?: string };
    ...
    try {
        ...
        if (role === "ADMIN") {
            siswaData = await prisma.dataSiswa.findMany({
                where: whereFilter,
                select: { id: true, userId: true, kelas: true, tempatPKL: true, guruPembimbing: true },
            });
            ...
            siswaData = await prisma.dataSiswa.findMany({
                where: {
                    ...whereFilter,
                    guruPembimbing: { equals: guruName, mode: "insensitive" },
                },
                select: { id: true, userId: true, kelas: true, tempatPKL: true, guruPembimbing: true },
            });
        ...
        const userIdToNameMap = new Map(users.map(u => [u.username ?? '', u.name ?? '']));
